...
class Page:
    def __init__(self, url, total_word_count=0, term_frequency_vector=set()):
        self.url = url
        self.word_count = total_word_count
        self.frequency_vector = term_frequency_vector


inverted_token_index = dict()  # Links word to Integer representing url
url_index = dict()  # Links Integer to Url
unique_words = set()  # Unique words found
pages = dict()
...
def get_links_and_index_html(url):
    ...
    filename = filename.replace(":", "")
    filename = "./pages/full-" + filename + ".html"
    ...
    # Add page to page indexing
    pages[url] = Page(url, total_word_count=total_words)
    ...
    for i in clean_text:
        unique_words.add(i)  # Track unique words
...
def clean_page_contents(contents):
    bad_characters = "\\n\\t\\r\")([]{}|<>'"
    replace_char = contents.maketrans("", "", bad_characters)
    clean_text = contents.translate(replace_char)
    clean_text = ' '.join(clean_text.split())
    return clean_text


def tokenize_and_stem(clean_text):
    clean_text = nltk.word_tokenize(clean_text)
    clean_text = [word for word in clean_text if word not in stopwords.words('english')]
    p_stem = p.PorterStemmer()
    clean_text = [p_stem.stem(word) for word in clean_text]
...
def build_tf_vectors():
    global pages
    global unique_words
    unique_words = sorted(unique_words)
    for page in pages:
        url = pages[page].url
        contents = read_page_contents(url)
        clean_text = clean_page_contents(contents)

        # Tokenize and Stem
        clean_text = tokenize_and_stem(clean_text)

        tf_vector = [0] * len(unique_words)
        for word in clean_text:
            try:
                term_id = unique_words.index(word)
                tf_vector[term_id] = tf_vector[term_id] + 1
            except ValueError:
                continue
        pages[page].frequency_vector = tf_vector


def build_engine_indexing():
    seed = ["https://en.wikipedia.org/wiki/William_Smith"]
    web_crawl(seed, 400, 1)

    build_tf_vectors()

    with open('unique_words.pickle', 'wb') as uw:
        pickle.dump(unique_words, uw)

    with open('pages.pickle', 'wb') as p:
        pickle.dump(pages, p)
...
def jaccard_similarity(search_query, document):
    intersection = set(search_query).intersection(set(document))
    union = set(search_query).union(set(document))
    return len(intersection) / len(union)


def engine_search(search_query, ranking_algo="JC"):
    global pages
    ...
    for word in search_query:
        ...
        if word in inverted_token_index:
            for url in inverted_token_index[word]:
                all_results.append(url_index[url])
                print(url_index[url] + " mentions " + word)

    # Tuples of (ranking, url) where a higher ranking is better
    ranked_pages = []

    # Jaccard Coefficient
    if ranking_algo == "JC":
        for result in all_results:
            document = read_page_contents(result)
            document = clean_page_contents(document)
            document = tokenize_and_stem(document)
            # document = nltk.word_tokenize(document)
            ranking = jaccard_similarity(search_query, document)
            ranked_page = (ranking, result)
    ...
    if ranking_algo == "TF-IDF":
        pass

    ranked_pages = sorted(ranked_pages, reverse=True)
    return ranked_pages
...
def load_engine():
    ...
    try:
        url_index_pickle = open('url_index.pickle', 'rb')
        url_index = pickle.load(url_index_pickle)
        inverted_token_index_pickle = open('inverted_token_index.pickle', 'rb')
        inverted_token_index = pickle.load(inverted_token_index_pickle)
        pages_pickle = open('pages.pickle', 'rb')
        pages = pickle.load(pages_pickle)
        unique_words_pickle = open('unique_words.pickle', 'rb')
        unique_words = pickle.load(unique_words_pickle)
...
def main():
    ...
    ranked_pages = engine_search(search_query)
